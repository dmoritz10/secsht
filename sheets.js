async function listSheet(title) {
  ...
  var objSht = await openShts([{ title: title, type: "all" }]);
  ...
  for (var i = 0; i < vals.length; i++) {
    ...
    if (shtEnc)
      vals[i].push(
        await decryptMessage(vals[i][0])
      ); // sort won't take a promise
  ...
  for (var j = 0; j < shtVals.length; j++) {
    ...
    ele
      .find("#btnShtFavorite")[0]
      .setAttribute("onclick", "setFavorite(" + j + ")");

    ele
      .find("#btnShtShowSheet")[0]
      .setAttribute("onclick", "showSheet(" + j + ")");
  ...
  if (srchVal) {
    $("#shtContainer #shtProvider").filter(function () {
      $(this)
        ...
        .parent()
        .toggle(
          $(this).text().toLowerCase().indexOf(srchVal.toLowerCase()) > -1
        );
    });
  }
...
async function setFavorite(arrIdx) {
  ...
  if (shtEnc) {
    ...
    if (fav) {
      shtVals[arrIdx][shtHdrs.indexOf("Favorite")] = await encryptMessage(
        "FALSE"
      );
    } else {
      shtVals[arrIdx][shtHdrs.indexOf("Favorite")] = await encryptMessage(
...
async function updateUI(valsEnc, arrIdx) {
  ...
  if (arrIdx == -1) {
    // add.  In this case, still use listSheet
  ...
  var $provider = $("#shtContainer > div")
    .find("#shtProvider")
    .eq(arrIdx + 1); // first ele is template d-none
