async function loadSheets() {
  ...
  if (sheets) {
    ...
    for (var j = 0; j < sheets.length; j++) {
      ...
      if (
        sht.gridProperties.columnCount == arrOptions["Nbr Columns per Sheet"] &&
        sht.title != "template"
      ) {
        var enc = await testEncrypted(sht.title, "Provider");
        ...
        if (enc.isSecSht) {
          ...
          if (enc.enc) {
            ...
            ele
              ...
              .setAttribute(
                "onclick",
                "decryptSheet('" + sht.title + "','Provider')"
              );
          } else {
            ...
            ele
              ...
              .setAttribute(
                "onclick",
                "encryptSheet('" + sht.title + "','Provider')"
              );
          }

          ele
            .find("#hmShowSheet")[0]
            .setAttribute("onclick", "listSheet('" + sht.title + "')");
...
async function btnHMChangePwdHtml() {
  ...
  for (const sht in secSht) {
    if (secSht[sht].enc) {
      var objSht = await openShts([{ title: sht, type: "all" }]);
...
async function verifyCurrPwd(pwdText, prmpt = "") {
  ...
  if (dx != pwdText) {
    await confirm("Invalid password");
    return null;
  }
...
async function btnNewSheetHtml() {
  ...
  var sht = await gapi.client.sheets.spreadsheets.sheets
    .copyTo(params, copySheetToAnotherSpreadsheetRequestBody)
  ...
  await gapi.client.sheets.spreadsheets.values.update(params, resource).then(
    function (response) {
      console.log("Sheet update successful");
    },
    function (reason) {
      console.error(
        'error updating sheet "' + "1" + '": ' + reason.result.error.message
      );
      alert(
        'error updating sheet "' + "1" + '": ' + reason.result.error.message
      );
